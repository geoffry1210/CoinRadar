...
// ─── /chart DATA SOURCE ─────────────────────────────────────────────────────
//
// chart-img.com's free tier only allowed a handful of tickers, so /chart is
// rebuilt on CoinGecko (already used elsewhere in this file, free, covers
// thousands of coins) for OHLC data, rendered as a candlestick image via
// QuickChart (also already used elsewhere for other charts).
//
// One real tradeoff: CoinGecko's free OHLC endpoint doesn't let you request
// candle granularity directly — it's tied to the `days` range (1 day → ~30
// min candles, 2–30 days → 4-hour candles, 31–90 days → 4-day candles, >90
// days → daily). So a requested interval like "1h" doesn't get literal
// 1-hour candles — it gets mapped to whatever range produces the closest
// available granularity, and the actual granularity is shown in the chart
// caption so it's never a silent mismatch.
function mapIntervalToCoinGeckoDays(interval) {
  const table = {
    '1M':  { days: 1,   label: '~30-min candles (last 24h)' },
    '5M':  { days: 1,   label: '~30-min candles (last 24h)' },
    '15M': { days: 1,   label: '~30-min candles (last 24h)' },
    '30M': { days: 1,   label: '~30-min candles (last 24h)' },
    '1H':  { days: 7,   label: '4-hour candles (last 7 days)' },
    '2H':  { days: 30,  label: '4-hour candles (last 30 days)' },
    '4H':  { days: 30,  label: '4-hour candles (last 30 days)' },
    '1D':  { days: 90,  label: '4-day candles (last 90 days)' },
    '1W':  { days: 365, label: 'daily candles (last 365 days)' },
  };
  return table[interval] || table['1D'];
}
...
async function fetchCoinGeckoOHLC(coinId, days) {
  ...
  if (!res.ok) throw new Error(`CoinGecko OHLC returned HTTP ${res.status}`);
  const rows = await res.json();
  if (!Array.isArray(rows) || rows.length === 0) throw new Error('CoinGecko returned no OHLC data for this coin/range');
  return rows; // [[timestamp, open, high, low, close], ...]
}

async function buildCandlestickChartBuffer(symbol, ohlcRows, granularityLabel) {
  const labels = ohlcRows.map(([ts]) => {
    const d = new Date(ts);
    return `${d.getMonth() + 1}/${d.getDate()} ${String(d.getHours()).padStart(2, '0')}:${String(d.getMinutes()).padStart(2, '0')}`;
  });
  const data = ohlcRows.map(([, o, h, l, c]) => ({ o, h, l, c }));
  ...
  const chartConfig = {
    ...
    options: {
      plugins: {
        title: { display: true, text: `${symbol} — ${granularityLabel}` },
  ...
  const res = await fetch('https://quickchart.io/chart', {
    ...
    body: JSON.stringify({
      ...
      version: '3', // required for chartjs-chart-financial (candlestick) support
    }),
  });
  if (!res.ok) throw new Error(`QuickChart returned HTTP ${res.status}`);
  return res.buffer();
}
...
//
// Implemented with AsyncLocalStorage so we don't have to touch every
// individual bot.sendMessage/bot.sendPhoto call site: we intercept
// bot.emit() for incoming messages and run the rest of that message's
// entire processing (all onText handlers, including anything they await)
// inside an async context carrying the triggering message. The wrapped
// sendMessage/sendPhoto below then read that context automatically.
...
bot.emit = function (event, ...args) {
  ...
  if (maybeMsg && maybeMsg.chat && maybeMsg.message_id) {
    if (maybeMsg.from?.id) recordKnownUser(maybeMsg);
    return messageReplyContext.run(maybeMsg, () => originalEmit(event, ...args));
  }
...
bot.onText(/\/chart (.+)/, async (msg, match) => {
  ...
  if (parts.length < 2) {
    return bot.sendMessage(
      ...
      '_Note: candle granularity depends on the timeframe range (CoinGecko\'s free data), not the exact interval — the chart caption always shows what you actually got._',
      { parse_mode: 'Markdown' }
    );
  }

  // Accept "/chart TICKER TIMEFRAME" (current) as well as the old
  // "/chart TICKER EXCHANGE TIMEFRAME" shape — exchange is ignored, since
  // CoinGecko's data isn't tied to a specific exchange.
  const symbol = parts[0];
  const interval = parts[parts.length - 1];

  bot.sendMessage(chatId, `📈 Fetching chart for *${symbol}* (${interval})...`, { parse_mode: 'Markdown' });

  try {
    const coinId = await resolveCoinGeckoId(symbol);
    if (!coinId) throw new Error(`couldn't find a CoinGecko listing for "${symbol}"`);

    const { days, label: granularityLabel } = mapIntervalToCoinGeckoDays(interval);
    const ohlcRows = await fetchCoinGeckoOHLC(coinId, days);
    const buffer = await buildCandlestickChartBuffer(symbol, ohlcRows, granularityLabel);

    await bot.sendPhoto(chatId, buffer, {
      caption: `📈 *${symbol}*\n_${granularityLabel} — data via CoinGecko_`,
      parse_mode: 'Markdown',
    });
  } catch (err) {
    console.error('/chart error:', err.message);
    bot.sendMessage(chatId, `📈 Couldn't generate a chart for *${symbol}* — ${err.message}`, { parse_mode: 'Markdown' });
  }
});
