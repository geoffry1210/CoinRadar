...
function wrapBoxed(text, label = 'COINRADAR') {
  const header = `<code>${BOX_TOP}\n┃  📡 ${label}  ┃\n${BOX_BOTTOM}</code>`;
  const body = markdownFragmentsToHtml(text);
  return `${header}\n\n<blockquote>${body}</blockquote>`;
}

// Same styling as wrapBoxed() but WITHOUT the "COINRADAR" header bar —
// just the blockquote box. Used by commands (like /p) that want the price
// box look without the repeated header on top of the coin image.
...
// (free public image proxy, no API key). Telegram sizes photo bubbles by
// aspect ratio at a fixed width, so a wide/short image renders with much
// less height than the original near-square logo — this is what actually
// makes the image look "smaller" in the chat.
function shrinkImage(url) {
  if (!url) return url;
  const encoded = encodeURIComponent(url);
  // w/h = final canvas size, fit=contain keeps the logo whole and centers
  // it, bg=white avoids a transparent-PNG-on-dark-theme issue.
  return `https://images.weserv.nl/?url=${encoded}&w=640&h=220&fit=contain&bg=white`;
}
...
bot.onText(/\/p (.+)/, async (msg, match) => {
  ...
  if (data.logoImage) bot.sendPhoto(chatId, shrinkImage(data.logoImage), { caption, parse_mode: 'HTML' });
