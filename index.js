  ...
  try {
    ...
    for (const item of items) {
      ...
      await broadcastToSubscribers(async (targetId) => {
        if (safetyResult?.image) await bot.sendPhoto(targetId, safetyResult.image, { caption, parse_mode: 'Markdown' });
        else await bot.sendMessage(targetId, caption, { parse_mode: 'Markdown' });
      });
