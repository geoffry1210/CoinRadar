...
// Pads/letterboxes a coin logo into a short, wide canvas via images.weserv.nl
// (free public image proxy, no API key), aligned to the left edge. Telegram
// sizes photo bubbles by aspect ratio at a fixed width, so a wide/short
// canvas renders with much less height than the original near-square logo,
// and left-aligning (instead of centering) keeps the visible artwork
// confined to roughly the left third of the bubble instead of spreading
// toward the middle.
// Fetches a coin logo and composites it, small and left-aligned, onto a
// white canvas — done locally with sharp instead of a third-party proxy
// (images.weserv.nl), which twice returned a 200 OK with a blank canvas
// and no logo actually composited in, with no way for us to detect that
// from the HTTP status alone. Doing this ourselves means we can see
// exactly why it failed if it ever does (network error, bad image data,
// etc.) instead of guessing at an opaque proxy's behavior.
const LOGO_CANVAS_W = 500;
const LOGO_CANVAS_H = 90;
const LOGO_SIZE     = 78;   // logo box size, small margin inside canvas height
const LOGO_MARGIN_L = 6;    // left inset from canvas edge

async function buildShrunkLogoBuffer(url) {
  const res = await fetch(url);
  if (!res.ok) throw new Error(`logo fetch failed: HTTP ${res.status}`);
  const inputBuffer = await res.buffer();
  ...
  return sharp({
    ...
    .composite([{
      input: resizedLogo,
      left: LOGO_MARGIN_L,
      top: Math.round((LOGO_CANVAS_H - LOGO_SIZE) / 2),
    }])
    .png()
    .toBuffer();
}

// Charts (TradingView/QuickChart snapshots) are rectangular data
// visualizations, not square logos — shrinking/letterboxing them the same
// way would make them illegible, so they're excluded from the automatic
// treatment in the sendPhoto wrapper below.
...
// Verifies a weserv.nl transform actually resolves before handing it to
// Telegram. Falls back to the original, unshrunk photo on any failure
// (timeout, non-200, network error) — a broken/blank image is worse than
// a full-size one, so this is a safety net against the proxy having a bad
// moment (as happened once already with an outdated parameter name).
async function safeShrinkImage(url) {
  if (!url) return url;
  try {
    return await buildShrunkLogoBuffer(url);
  } catch (err) {
    console.error('buildShrunkLogoBuffer failed, sending original image instead —', err.message);
    return url;
  }
}
...
bot.sendPhoto = async function (chatId, photo, options = {}) {
  ...
  const finalPhoto = isChartImage(photo) ? photo : await safeShrinkImage(photo);
  const finalOptions = withAutoReply(chatId, { ...options, caption: finalCaption, parse_mode: 'HTML' });
  return originalSendPhoto(chatId, finalPhoto, finalOptions);
};
