...
function shrinkImage(url) {
  ...
  // Small logo box (90x90) inside a wider canvas (500), anchored left —
  // `a=left` controls weserv's positioning of the resized image within
  // the padded canvas. NOTE: the background param for fit=contain on the
  // current weserv API is `cbg=`, not `bg=` (that's a deprecated v4 name
  // that no longer does anything) — using the wrong one was likely why
  // the image came through completely blank.
  return `https://images.weserv.nl/?url=${encoded}&w=500&h=90&fit=contain&a=left&cbg=white`;
}
...
async function safeShrinkImage(url) {
  const shrunk = shrinkImage(url);
  try {
    const controller = new AbortController();
    const timeout = setTimeout(() => controller.abort(), 4000);
    const res = await fetch(shrunk, { method: 'HEAD', signal: controller.signal });
    clearTimeout(timeout);
    if (res.ok) return shrunk;
    console.error(`safeShrinkImage: weserv returned ${res.status}, falling back to original`);
  } catch (err) {
    console.error('safeShrinkImage: weserv unreachable, falling back to original —', err.message);
  }
  return url;
}
